...
class FaceIdentityManager:
    ...
    def _update_embedding(self, person_id: str, new_embedding: np.ndarray) -> None:
        """EMA blend of new embedding into in-memory cache only (no DB write).

        Skipping the per-match DB write avoids a synchronous SQLite transaction
        on every recognized face in every sampled frame — a major latency source
        at mass events.  The EMA-weighted embedding is still updated in RAM so
        the matcher adapts to lighting/angle drift within the session.
        """
        alpha = 0.1
        for known_face in self._known:
            if known_face.person_id == person_id:
                known_face.embedding = (
                    (1.0 - alpha) * known_face.embedding + alpha * new_embedding
                )
                break
    ...
    def _compute_embedding(self, face_rgb: np.ndarray) -> Optional[np.ndarray]:
        """Encode a pre-cropped RGB face into a 128-d dlib embedding.

        Tuned for throughput at mass events: upsampling=0, jitters=1,
        model='small'.  The upstream YOLO detector already provides tight
        face crops so a second face_locations scan is still needed to
        filter detector false positives, but we skip the expensive upsampling.
        """
